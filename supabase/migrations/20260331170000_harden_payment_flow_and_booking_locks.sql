...
create index if not exists bookings_host_created_at_idx
on public.bookings (host_id, created_at desc);

create index if not exists booking_email_logs_invoice_id_idx
on public.booking_email_logs (invoice_id);
...
revoke all on function public.book_parking_spot_paid(uuid, uuid, timestamptz, timestamptz, numeric, text) from public, anon, authenticated;
grant execute on function public.book_parking_spot_paid(uuid, uuid, timestamptz, timestamptz, numeric, text) to service_role;

revoke all on function public.book_parking_spot(uuid, timestamptz, timestamptz, numeric) from public, anon, authenticated;
